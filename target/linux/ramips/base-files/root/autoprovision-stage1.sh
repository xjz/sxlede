...
# autoprovision stage 1: this script will be executed upon boot without a valid extroot (i.e. when rc.local is 
# found and run from the internal overlay)
...
setupExtroot()
{
    ...
    mkdir -p ${overlay_root}/etc/config
    cp -f /root/fstab ${overlay_root}/etc/config/fstab
    cp -f /root/fstab /etc/config/fstab
